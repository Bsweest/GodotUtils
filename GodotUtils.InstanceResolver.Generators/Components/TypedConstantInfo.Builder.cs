...
partial record TypedConstantInfo
{
    ...
    public static TypedConstantInfo Create(TypedConstant arg)
    {
        ...
        {
            string elementTypeName = ((IArrayTypeSymbol)arg.Type!).ElementType.ToDisplayString(
        ...
        return (arg.Kind, arg.Value) switch
        {
            ...
            (TypedConstantKind.Primitive, object value)
                => value switch
                {
    ...
    )
    {
        ...
        {
            info = new Type(
                typeOfOperation.TypeOperand.ToDisplayString(
                    SymbolDisplayFormat.FullyQualifiedFormat
                )
            );
        ...
        if (operation is IArrayCreationOperation)
        {
            string? elementTypeName = (
                (IArrayTypeSymbol?)operation.Type
            ...
            {
                if (
                    semanticModel.GetOperation(initializationExpression, token)
                    is not IOperation initializationOperation
                )
                {
                    goto Failure;
                }
                ...
                if (
                    !TryCreate(
                        ...
                        token,
                        out TypedConstantInfo? elementInfo
                    )
                )
                {
                    goto Failure;
